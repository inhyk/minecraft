...
### 온라인 4인 플레이
- 정적 웹페이지에서 PeerJS/WebRTC로 브라우저들을 연결하고 호스트가 최대 세 손님의 상태를 중계
- 호스트가 만든 6자리 방 코드로 친구들이 입장
- 호스트 포함 최대 4명이며 월드 시드, 블록, 플레이어, 몹 상태를 동기화
- 호스트 플레이어(첫 번째 접속자)가 몹 시뮬레이션을 실행하고 다른 플레이어에게 동기화
- 주요 이벤트: `player_join`, `player_move`, `player_leave`, `block_set`, `mob_state`, `chat`
